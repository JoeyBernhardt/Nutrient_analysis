...
calcium <- traits1b %>% 
  ...
  mod1a <- lm(log_concentration ~ bulk_trophic_level + log_length  + feeding_mode + DemersPelag+ part,  data = calcium)  
  stargazer(mod1a, title = "", type = "html", out="tables/large-ca-models-expanded-all-parts-ols.htm", ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
...
cal_data <- calg2
### full model
mod1a <- gls(log_concentration ~ bulk_trophic_level + log_length  + feeding_mode + DemersPelag + part, 
             correlation = corPagel(value = 0, phy = tree, fixed = TRUE), data = calg2, method = "ML")
...
stargazer(mod1a, title = "", type = "html", out="tables/small-fe-part-models-expanded-all-parts-pgls-lambda.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
