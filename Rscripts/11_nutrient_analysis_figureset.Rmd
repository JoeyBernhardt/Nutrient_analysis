...
ggplot(data = ., aes(x = number_of_species, y = number_of_targets, color = group)) + geom_line(size =1, linetype = "dotted") + theme_bw() + geom_ribbon(aes(ymin = number_of_targets - se, ymax = number_of_targets + se), alpha = 0.2, size = 1) + ylab("number of DRI targets") +
  xlab("number of species") + theme(text = element_text(size=24)) + scale_colour_grey()
...
ggsave("/Users/Joey/Documents/Nutrient_Analysis/figures/concentration_body_size.png", width = 8, height = 6)
```
...
ggplot(data = microelements, aes(x = term, y = slope)) + geom_point(size = 4) + 
