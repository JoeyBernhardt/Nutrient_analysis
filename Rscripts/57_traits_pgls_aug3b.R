...
filter(nutrient %in% c("epa", "dha", "ca_mg", "fe_mg", "zn_mg")) %>% 
  filter(!is.na(concentration)) %>% 
  mutate(datasource1 = "seanuts")
unique(traits_nut$part)

traits_nut %>% 
  filter(reference == "27") %>% View
  ...
  filter(!is.na(concentration)) %>% 
  mutate(datasource1 = "mtraits") 

  
  length(unique(mtraits$species1))
  
  length(unique(traits_nut$species1))

  setdiff(unique(mtraits$species1), unique(traits_nut$species1))
  setdiff(unique(traits_nut$species1), unique(mtraits$species1))
...
traits1b %>% 
  filter(grepl("trachurus", species1)) %>% 
  filter(nutrient == "ca_mg") %>% 
  distinct(datasource1, concentration) %>% View
traits_to_pub <- traits1b %>% 
  filter(nutrient %in% c("epa", "dha", "ca_mg", "fe_mg", "zn_mg")) %>% 
  filter(!is.na(concentration)) %>% 
  ...
  group_by(species1, feeding_mode, EnvTemp, DemersPelag, BodyShapeI, part, realm, feeding_level, nutrient) %>%
...
stargazer(mod1a, title = "", type = "html", out="tables/calcium-models-expanded-muscle-only-pgls-lambda-muscleskin4-aug33.htm", 
...
stargazer(mod1a, title = "", type = "html", out="tables/iron-models-expanded-muscle-only-pgls-muscleskin4-aug3.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
...
stargazer(mod1a, title = "", type = "html", out="tables/epa-models-expanded-muscle-only-pgls-muscle-skin-aug3.htm", 
...
stargazer(mod1a, title = "", type = "html", out="tables/1iron-models-expanded-all-parts-pgls-lambda-aug3.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
...
rsq_mod1a <- round(rsquared(mod1a)['R.squared'][[1]], digits = 2)
summary(mod1a)
stargazer(mod1a, title = "", type = "html", out="tables/epa-models-expanded-all-parts-pgls-lambda-aug3b.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
...
stargazer(mod1a, title = "", type = "html", out="tables/dha-models-expanded-all-parts-pgls-lambda-aug3.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2, single.row = TRUE)
