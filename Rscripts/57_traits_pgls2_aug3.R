...
traits2b <- read_csv("data-processed/trait-nutrient-data-analysis-aug3-cine-removed.csv")
...
### full model
mod1a <- gls(log_concentration ~ bulk_trophic_level + log_length  + feeding_mode + DemersPelag + BodyShapeI + realm, 
...
calcium <- traits2 %>% 
  ...
  filter(part != "unspecified") %>% 
  filter(part %in% c("muscle", "muscle + skin")) %>%
  group_by(species1, feeding_mode, DemersPelag, BodyShapeI, part, realm) %>%
  summarise_each(funs(mean), log_concentration, log_length, bulk_trophic_level) %>% 
  ungroup() %>%
  filter(complete.cases(.))
str(calcium)
...
write_csv(all_cf, "data-processed/all-cfs-traits-pgls.csv")
...
ggsave("figures/coef-all-micronutrients-2new.png", width = 8, height = 10)
...
stargazer(mod1a, title = "", type = "html", out="tables/epa-models-expanded-all-parts-pgls-aug3.htm", 
          add.lines = list(c("R2", rsq_mod1a), c("Lamba", lambda)), ci=TRUE, ci.level=0.95, digits = 2)
